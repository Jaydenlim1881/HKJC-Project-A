...
def generate_month_dates(year: int, month: int):
    """Yield datetime objects for each day in the given month."""
...
def main():
    driver = initialize()
    all_rows = []   # ✅ make sure this is defined before using it
    try:
        # Loop through all days in the month
        for dt in generate_month_dates(SCRAPE_YEAR, SCRAPE_MONTH):
            date_api = dt.strftime('%Y/%m/%d')   # for scraping URL
            date_log = dt.strftime('%d/%m/%y')   # for logs

            for course in ['ST', 'HV']:
                for race_num in range(1, 12):
                    try:
                        race_data = scrape_race(driver, date_api, course, race_num)
                        if race_data:
                            all_rows.extend(race_data)
                            logger.info(f"✅ {date_log} {course} R{race_num}: +{len(race_data)} rows")
                    except Exception as e:
                        logger.error(f"⚠️ {date_log} {course} R{race_num} failed: {e}")
                        continue

        if not all_rows:
            logger.warning("⚠️ No data collected for the month.")
            return
        ...
        csv_path = f"{CONFIG['output_dir']}/races_{SCRAPE_YEAR}_{SCRAPE_MONTH:02d}.csv"
